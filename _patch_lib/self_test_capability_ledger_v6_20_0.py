...
master=(HERE/'self_test_python_patch_tool_v6_20_0.py').read_text(); assert 'self_test_historical_compatibility_v6_20_0.py' in master and 'self_test_capability_ledger_v6_20_0.py' in master
...
for token in ['COLLECT capability continuity','search_files','symbol_graph']:
    assert token.lower() in policy.lower(),token
assert 'self_test_collect_historical_actions_v6_20_0.py' in master
...
# v6.20.0 additive database SELECT capability is release-gated too.
...
assert (DOC/'DATABASE_SELECT_ACTIVE_BUILDER.md').is_file()
assert (ROOT/'tools'/'db_profiles.example.json').is_file()
for token in ['database_select','active builder','raw SQL','self_test_database_select_v6_20_0.py']:
...
for token in ['clear-text companion','COLLECT result','FAIL_HANDOFF']:
    assert token.lower() in ledger.lower(), token


# v6.20.0 additive AI tool-context synchronization capability is release-gated.
...
assert 'ai_context' in schema['manifest']['allowed_fields']
for token in ['AI tool-context synchronization','AI_TOOL_SYNC','sync_token','self_test_ai_sync_v6_20_0.py']:
    assert token.lower() in ledger.lower(), token
assert 'self_test_ai_sync_v6_20_0.py' in master


# v6.20.0 copy-friendly upload-path continuity is release-gated.
...
for token in ['copy-friendly','ptv_to_ai','hard-link','self_test_copyable_upload_path_v6_20_0.py']:
    assert token.lower() in ledger.lower(), token
assert 'self_test_copyable_upload_path_v6_20_0.py' in master



# v6.20.0 safe Git and manual-execution transition is explicit and behavioral.
...
assert by_id[8]['disposition']=='REMOVED_BY_REQUIREMENT'
assert by_id[43]['disposition']=='PRESERVED'
assert (HERE/'python_patch_git_safe.py').is_file()
assert (HERE/'python_patch_manual_workflow.py').is_file()
assert (DOC/'GIT_SAFE_OPERATIONS.md').is_file()
assert (DOC/'MANUAL_EXECUTION_WORKFLOW.md').is_file()
for token in ['safe Git','manual execution','REMOVED_BY_REQUIREMENT','self_test_git_safe_v6_20_0.py','self_test_manual_execution_v6_20_0.py']:
    assert token.lower() in ledger.lower(), token
assert 'self_test_git_safe_v6_20_0.py' in master
assert 'self_test_manual_execution_v6_20_0.py' in master

print('PASS: v6.20.0 cumulative capability ledger and no-silent-removal governance are packaged and release-gated')
