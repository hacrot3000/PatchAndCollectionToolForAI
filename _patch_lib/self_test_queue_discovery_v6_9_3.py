...
with tempfile.TemporaryDirectory() as td:
    ...
    with zipfile.ZipFile(p/'python_patch_tool_v6.9.3.zip','w') as z:
    ...
    for rejected in ['PTV_PASS_HANDOFF.zip','PTV_REALISTIC_HANDOFF.zip','PTV_HANDOFF_WITH_COLLECT_REQUEST.zip','python_patch_tool_v6.9.3.zip','wrapped_python_patch_tool.zip','notes.py','broken.zip','linked_patch.py','tool_distribution.tgz','handoff_archive.tgz','wrapped_handoff_archive.tgz','CODE_COLLECTION_REQUEST_loose.json']:
    ...
    assert 'python_patch_tool_v6.9.3.zip' in joined
    assert 'broken.zip' in joined
    assert 'SKIPPED symlink queue entry: patchs/linked_patch.py' in joined

print('PASS: v6.9.3 queue recognizes PATCH/COLLECT structurally and skips non-patch artifacts')
