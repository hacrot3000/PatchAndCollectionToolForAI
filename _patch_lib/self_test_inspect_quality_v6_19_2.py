...
with tempfile.TemporaryDirectory(prefix='ptv613_inspect_') as td:
    ...
    manifest={'schema_version':1,'patch':{'id':'inspect'},'targets':['target.txt'],'compatibility':{'min_tool_version':'6.12.0','max_tested_version':'6.19.2'}}
...
with tempfile.TemporaryDirectory(prefix='ptv613_quality_') as td:
    ...
    assert 'COLLECT INCOMPLETE' in cp.stdout
    assert 'SUMMARY: INCOMPLETE | incomplete=1 | failed=0' in cp.stderr
    ...
    assert last['status']=='INCOMPLETE' and last['exit_code']==3
    quality=last['results'][0]['collect_result']['quality']; assert quality['truncated_reports']==1 and quality['missing']==0
print('PASS: v6.19.2 inspect/dry-run and COLLECT quality summary')
