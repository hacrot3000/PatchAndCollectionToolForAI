...
assert m.VERSION=='6.15.1'

def make_patch(path: Path) -> None:
    path.parent.mkdir(parents=True,exist_ok=True)
    with zipfile.ZipFile(path,'w') as zf:
        zf.writestr('PATCH_TOOL_MANIFEST.json','{"schema_version":1,"project":{"id":"x"},"patch":{"id":"x"}}')
...
with tempfile.TemporaryDirectory(prefix='ptv6151_result_fail_') as td:
    ...
    assert 'SUMMARY: FAIL | stopped after patch_visual_fail.zip rc=7' in text,text
...
print('PASS: v6.15.1 final result clarity, patch highlighting and red/yellow FAIL banner contract')
