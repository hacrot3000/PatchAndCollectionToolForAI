...
assert version=='6.10.1',version
for rel in ['python_patch_queue_dispatcher.py','python_patch_collect_progress_v6_7.py']:
    text=(root/rel).read_text(encoding='utf-8')
    assert 'VERSION = "6.10.1"' in text,(rel,version)
launcher=(root.parent/'run_python_patches.sh').read_text(encoding='utf-8')
assert 'v6.10.1' in launcher
master=(root/'self_test_python_patch_tool_v6_10_1.py').read_text(encoding='utf-8')
assert 'self_test_collect_progress_v6_10_1.py' in master
assert 'self_test_local_duplicate_v6_10_1.py' in master
assert 'self_test_collect_exclusivity_v6_10_1.py' in master
...
print('PASS: v6.10.1 executable version markers and master coverage are synchronized')
