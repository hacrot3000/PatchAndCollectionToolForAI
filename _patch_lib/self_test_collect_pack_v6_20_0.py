...
FILES=['python_patch_collect_progress_v6_7.py','python_patch_collect_compat.py','python_patch_collect_schema.py','python_patch_git_safe.py','python_patch_collect_regex_worker.py','python_patch_database_select.py','python_patch_cleartext_companion.py','python_patch_ai_sync.py']
...
with tempfile.TemporaryDirectory(prefix='ptv612_pack_') as td:
    ...
    with zipfile.ZipFile(results[0]) as z:
        ...
        assert manifest['tool_version']=='6.20.0' and manifest['file_count']==2
...
# Truly unknown action still fails exact-schema preflight; restored historical actions are valid.
with tempfile.TemporaryDirectory(prefix='ptv612_schema_bad_') as td:
    ...
    make_request(req,{'id':'bad','actions':[{'type':'definitely_unknown_collect_action','paths':['.']}]})
    cp=run_collect(root,launcher,name); assert cp.returncode==2,(cp.stdout,cp.stderr)
    assert 'unsupported action type: definitely_unknown_collect_action' in cp.stdout+cp.stderr,cp.stdout+cp.stderr
    assert req.exists()

print('PASS: v6.20.0 self-contained COLLECT current/restored action schema and unknown-action fail-closed')
