...
with tempfile.TemporaryDirectory(prefix='ptv617_prev_required_') as td:
    ...
    # Force newest to choose only successor, leaving failed predecessor unresolved.
    config(r,selection='newest')
    make_patch(r,'patch_2.zip','new','from pathlib import Path\nPath("new.txt").write_text("x")\n',targets=['new.txt'])
    cp=run(r); assert cp.returncode==2,(cp.stdout,cp.stderr)
    assert 'previous_failure' in (cp.stdout+cp.stderr) and not (r/'new.txt').exists()
...
with tempfile.TemporaryDirectory(prefix='ptv617_history_cleanup_') as td:
    ...
    for i in range(35):
        rid=f'h{i:02d}'
        m._write_run_report(r,{'run_id':rid,'started_at':f'2026-08-09T00:00:{i:02d}+00:00','status':'PASS','exit_code':0,'results':[]})
        if i==0:
...
print('PASS: v6.17.8 controlled continue, dependencies, whole-batch preflight, atomic rollback/requeue, predecessor action, smart resume, source diff, support bundle and history management')
