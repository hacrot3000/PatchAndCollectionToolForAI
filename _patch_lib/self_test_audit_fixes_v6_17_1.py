...
# internal_error is never safe to continue even if an older result accidentally says detected=false.
...
assert ok is False and reason=='internal_error',(ok,reason)
...
with tempfile.TemporaryDirectory(prefix='ptv6171_regex_') as td:
    ...
    report=c._search_action(root,action,dict(cs.DEFAULT_LIMITS))
    assert 'Matches: 1' in report and 'x.txt:1' in report,report
...
print('PASS: v6.17.1 audit regressions cover rollback completeness, failure stop, COLLECT self-output, mutation integrity, idempotency, Git, archives and limits')
