...
FILES=['python_patch_collect_progress_v6_7.py','python_patch_collect_compat.py','python_patch_collect_schema.py','python_patch_collect_regex_worker.py','python_patch_database_select.py','python_patch_cleartext_companion.py']
...
with tempfile.TemporaryDirectory(prefix='ptv612_pack_') as td:
    ...
    with zipfile.ZipFile(results[0]) as z:
        ...
        assert manifest['tool_version']=='6.19.1' and manifest['file_count']==2
...
print('PASS: v6.19.1 self-contained COLLECT current/restored action schema and unknown-action fail-closed')
