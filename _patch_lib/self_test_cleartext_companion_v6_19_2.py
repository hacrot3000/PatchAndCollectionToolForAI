...
assert c.VERSION==q.VERSION=='6.19.2'
...
with tempfile.TemporaryDirectory(prefix='ptv_cleartext_upload_') as td:
    ...
    out=buf.getvalue(); assert f'ZIP (preferred): {zip_path}' in out and f'Clear-text TXT: {txt_path}' in out
...
with tempfile.TemporaryDirectory(prefix='ptv_cleartext_history_') as td:
    ...
    manifest={'format':'python-patch-tool-code-collection','format_version':3,'tool_version':'6.19.2','collection_status':'PASS','files':[],'reports':[]}
...
print('PASS: v6.19.2 ZIP clear-text companions for COLLECT and FAIL_HANDOFF')
