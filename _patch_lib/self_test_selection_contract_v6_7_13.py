...
with tempfile.TemporaryDirectory(prefix='ptv678line_') as td:
    ...
    for n in ['patch_1.zip','patch_2.zip','patch_3.zip']:
        (p/n).write_text('x',encoding='utf-8'); items.append(m._queue_item(p/n,'PATCH'))
...
# Destructive delete must not unlink a same-named artifact that replaced the
# item after the selector snapshot was created.
with tempfile.TemporaryDirectory(prefix='ptv6713delete_toctou_') as td:
    root=Path(td); q=root/'patchs'; q.mkdir()
    target=q/'patch_replace.zip'
    target.write_text('old',encoding='utf-8')
    item=m._queue_item(target,'PATCH','legacy')
    items=[item]
    target.unlink(); target.write_text('NEW IMPORTANT ARTIFACT',encoding='utf-8')
    selected,deleted,failures=m._delete_indexes(root,items,{0},{0})
    assert target.exists(), 'replacement file must never be deleted from stale selector state'
    assert not deleted, deleted
    assert failures and 'replaced or modified' in failures[0], failures
    assert items and items[0].name=='patch_replace.zip', items

print('PASS: v6.7.13 historical selector/config contracts restored')
