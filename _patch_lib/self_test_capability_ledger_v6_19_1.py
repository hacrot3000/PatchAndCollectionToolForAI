...
master=(HERE/'self_test_python_patch_tool_v6_19_1.py').read_text(); assert 'self_test_historical_compatibility_v6_19_1.py' in master and 'self_test_capability_ledger_v6_19_1.py' in master
...
for token in ['COLLECT capability continuity','search_files','symbol_graph']:
    assert token.lower() in policy.lower(),token
assert 'self_test_collect_historical_actions_v6_19_1.py' in master
...
for token in ['Optional controlled installer','user_not_selected','Selection-aware automatic identity adoption']:
    assert token in ledger,token
assert 'self_test_portable_installer_v6_19_1.py' in master

# v6.19.1 additive database SELECT capability is release-gated too.
...
for token in ['database_select','active builder','raw SQL','self_test_database_select_v6_19_1.py']:
...
print('PASS: v6.19.1 cumulative capability ledger and no-silent-removal governance are packaged and release-gated')
