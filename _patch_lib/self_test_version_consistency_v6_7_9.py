...
assert version=='6.7.9',version
for rel in ['python_patch_queue_dispatcher.py','python_patch_collect_progress_v6_7.py']:
    text=(root/rel).read_text(encoding='utf-8')
    assert 'VERSION = "6.7.9"' in text,(rel,version)
launcher=(root.parent/'run_python_patches.sh').read_text(encoding='utf-8')
assert 'v6.7.9' in launcher
master=(root/'self_test_python_patch_tool_v6_7_9.py').read_text(encoding='utf-8')
assert 'self_test_collect_progress_v6_7_9.py' in master
...
print('PASS: v6.7.9 executable version markers and master coverage are synchronized')
