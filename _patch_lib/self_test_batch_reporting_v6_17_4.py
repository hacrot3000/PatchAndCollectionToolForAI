...
# Current default execution policy is fail-fast: mixed results are represented
# as PASS(es), one FAIL, then NOT_EXECUTED items rather than pretending later
# patches ran.  The report renderer must make that distinction explicit.
mixed={
...
# v6.17.4 keeps fail-fast as the default execution contract.
all_failed={
...
with tempfile.TemporaryDirectory(prefix='ptv6160_e2e_pass_') as td:
    ...
    assert idle_last['status']=='IDLE',idle_last
...
# End-to-end mixed batch preserves fail-fast: prior PASS, first FAIL, later
# selected PATCH is NOT_EXECUTED and remains queued for resume.
...
print('PASS: v6.17.4 persistent batch summary, aggregate/detail logs, fail-fast status model and report browser')
