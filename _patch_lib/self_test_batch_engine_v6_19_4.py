...
def run(root: Path, *args):
    ...
    return subprocess.run([str(root/'tools'/'run_python_patches.sh'),*args],cwd=root,text=True,capture_output=True,stdin=subprocess.DEVNULL,env=env,timeout=80)
...
print('PASS: v6.19.4 controlled continue, dependencies, whole-batch preflight, atomic rollback/requeue, predecessor action, smart resume, source diff, support bundle and history management')
