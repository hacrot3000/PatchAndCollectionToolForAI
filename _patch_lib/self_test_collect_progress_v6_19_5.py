...
with tempfile.TemporaryDirectory(prefix="ptprog_v6710_") as td:
    ...
    assert '/artifacts/ptv_to_ai/CR_' in cp.stdout, cp.stdout
    assert str(result) not in [line.strip() for line in cp.stdout.splitlines() if '/artifacts/ptv_to_ai/' in line], cp.stdout
...
with tempfile.TemporaryDirectory(prefix="ptprog_result_v6711_") as td:
    ...
    assert '/artifacts/ptv_to_ai/CR_' in out, out
    assert m._LAST_COLLECT_SUCCESS_META.get('result_zip') == str(result), m._LAST_COLLECT_SUCCESS_META
...
# displayed alias path remains complete/copyable on its dedicated row.
...
with tempfile.TemporaryDirectory(prefix="ptprog_fallback_v6711_") as td:
    ...
    out = capture.getvalue()
    assert ok is True, out
    assert '/artifacts/ptv_to_ai/CR_' in out and str(stale) not in out, out
    assert m._LAST_COLLECT_SUCCESS_META.get('result_zip') == str(valid), m._LAST_COLLECT_SUCCESS_META
...
print('PASS: Python Patch Tool v6.19.5 collect progress/artifact robustness self-test')
