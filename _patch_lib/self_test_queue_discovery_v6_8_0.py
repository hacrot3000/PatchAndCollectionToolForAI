...
with tempfile.TemporaryDirectory() as td:
    ...
    with zipfile.ZipFile(p/'python_patch_tool_v6.8.0.zip','w') as z:
    ...
    for rejected in ['PTV_PASS_HANDOFF.zip','PTV_REALISTIC_HANDOFF.zip','python_patch_tool_v6.8.0.zip','notes.py','broken.zip','linked_patch.py','tool_distribution.tgz','handoff_archive.tgz','CODE_COLLECTION_REQUEST_loose.json']:
        assert rejected not in by_name, (rejected,items,w)
    joined='\n'.join(w)
    assert 'RAW JSON REJECTED' in joined
    assert 'PTV_PASS_HANDOFF.zip' in joined
    assert 'python_patch_tool_v6.8.0.zip' in joined
    assert 'broken.zip' in joined
    assert 'SKIPPED symlink queue entry: patchs/linked_patch.py' in joined

print('PASS: v6.8.0 queue recognizes PATCH/COLLECT structurally and skips non-patch artifacts')
