...
# v6.20.2: even an internal/tool failure may continue independent work only when
...
with tempfile.TemporaryDirectory(prefix='ptv6171_regex_') as td:
    ...
    try:
        ...
        timed=c._search_action(root,bad,dict(cs.DEFAULT_LIMITS))
        assert timed.get('incomplete') is True,timed
        assert timed.get('coverage_status') == 'PARTIAL',timed
        assert any('hard timeout' in x for x in timed.get('reasons',[])),timed
    finally:
...
# v6.20.2 requirement-driven retirement: PATCH Git add/commit/push is rejected
...
for field,value in [('add','changed'),('commit','auto'),('push','auto')]:
    manifest={'schema_version':1,'patch':{'id':'audit-git'},'git':{'add':'off','commit':'off','push':'off'}}
    manifest['git'][field]=value
    if field=='commit': manifest['git']['commit_message']='audit'
    try: validate_manifest(manifest)
    except PatchSchemaError as exc: assert exc.kind=='git_mutation_forbidden'
...
print('PASS: v6.20.2 audit regressions cover rollback completeness, contained-failure continuation, COLLECT self-output, mutation integrity, idempotency, Git, archives and limits')
