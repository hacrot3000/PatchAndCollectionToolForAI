...
# v6.20.0 removes PATCH Git mutation execution by explicit requirement.
...
try:
    validate_manifest({'schema_version':1,'patch':{'id':'exec-audit-git'},'git':{'add':'changed','commit':'auto','commit_message':'x','push':'off'}})
except PatchSchemaError as exc:
    assert exc.kind=='git_mutation_forbidden'
else:
    raise AssertionError('retired PATCH Git mutation policy was accepted')
...
print("PASS: v6.20.0 failure-only commands and script-execution audit regressions")
