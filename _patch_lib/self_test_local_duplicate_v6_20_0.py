...
with tempfile.TemporaryDirectory(prefix='ptv680dup_split_') as td:
    ...
    # v6.20.0 retires confirmed local duplicates into patchs/ignore immediately.
...
# v6.20.0 hardens the whole queue boundary: patchs/ itself must never be a
...
# v6.20.0 deliberately has no project/process queue lock. Independent terminal
...
print('PASS: v6.20.0 current-session duplicate collapse plus local-history duplicate contract')
