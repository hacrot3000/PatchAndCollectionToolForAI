...
for phrase in ['BATCH REPORT + AGGREGATE/DETAIL LOG VIEWER','batch report acceptance','Multi-error','SOURCE_DRIFT','Windows fullscreen','patchs/ignore/YYYY-MM-DD','nền đỏ + chữ vàng','NOT_EXECUTED','batch.log']:
    assert phrase in impl,phrase
for phrase in ['Fullscreen Windows selector','Multi-error schema lint','validate --patch','OPS sequential dry-run','taskkill /T /F','PATCH_PACKAGE_CHECKLIST.json','báo 1 lần rồi move','FAIL banner nền đỏ/chữ vàng','Batch result table','aggregate `batch.log`','report --list']:
    assert phrase in features,phrase
assert patch_schema['tool_version']=='6.16.0' and patch_schema['schema_version']==1
assert checklist['tool_version']=='6.16.0' and 'READY_TO_APPLY' in checklist['result_classes']
assert collect_schema['tool_version']=='6.16.0'
for phrase in ['tools\\run_python_patches.bat','.\\tools\\run_python_patches.ps1','Python 3.10+','PATCH_PACKAGE_SCHEMA.json','PATCH_PACKAGE_CHECKLIST.json','source_baseline','FAIL_HANDOFF','v6.16.0 diagnostics contract']:
    assert phrase in ai,phrase
for phrase in ['tools\\run_python_patches.bat','.\\tools\\run_python_patches.ps1','Expand-Archive','Python 3.10+','fullscreen selector','validate --patch','report --list','batch.log','NOT_EXECUTED']:
    assert phrase in portable,phrase
for phrase in ['Windows internal PATCH/COLLECT routing without Bash','Windows fullscreen selector','Multi-error manifest lint','Sequential data-only OPS dry-run','Per-run batch summary','Interactive/reopenable `report`']:
    assert phrase in status,phrase
for phrase in ['Metadata-driven safe rollback','recovery.rollback.targets','Rollback path/runtime safety','Exact input lifecycle','v6.16.0 package lint / validate','source_baseline','READY_TO_APPLY']:
...
assert 'PASS / FAIL / NOT_EXECUTED / SKIPPED' in html and 'run_python_patches.bat report' in html and 'artifacts/patch_tool/runs/' in html
...
print('PASS: v6.16.0 diagnostics, Windows parity docs, schemas/checklist and minimal user HTML contract')
