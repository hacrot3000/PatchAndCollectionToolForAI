...
assert 'v6.19.2' in pst and 'v6.19.2' in bt
for phrase in [
...
if pwsh:
    ...
    assert cp.returncode==0,(cp.stdout,cp.stderr)
    assert '6.19.2' in cp.stdout+cp.stderr,(cp.stdout,cp.stderr)

print('PASS: v6.19.2 Windows BAT/PowerShell launcher contract and optional native PowerShell smoke test')
