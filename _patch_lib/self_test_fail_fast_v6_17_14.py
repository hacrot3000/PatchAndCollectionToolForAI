...
with tempfile.TemporaryDirectory(prefix='ptv678ff_') as td:
    ...
    rc,executed,remaining,late_duplicates,duplicate_warnings=m.execute_items(root,chosen,failure_policy="fail_fast")
    assert rc==9,rc
...
with tempfile.TemporaryDirectory(prefix='ptv678ffmain_') as td:
    ...
    launcher.chmod(0o755)
    install_shims(root)
    (root/'.python_patch_tool.json').write_text('{"batch":{"failure_policy":"fail_fast"}}',encoding='utf-8')
...
with tempfile.TemporaryDirectory(prefix='ptv6711sig_') as td:
    ...
    launcher.chmod(0o755)
    install_shims(root)
    rc,executed,remaining,late_duplicates,duplicate_warnings=m.execute_items(root,[m.QueueItem('patch_signal.zip','PATCH')],failure_policy="fail_fast")
...
with tempfile.TemporaryDirectory(prefix='ptv6711collectarchive_') as td:
    ...
    install_shims(root)
    rc,executed,remaining,late_duplicates,duplicate_warnings=m.execute_items(root,[m.QueueItem(request.name,'COLLECT')],failure_policy="fail_fast")
...
with tempfile.TemporaryDirectory(prefix="ptv691_summary_dup_") as td:
    ...
    rc,executed,remaining,late_dups,warns=m.execute_items(root,chosen,failure_policy="fail_fast")
    assert rc==0 and len(executed)==1 and len(late_dups)==1 and not remaining and not warns

print('PASS: v6.17.14 fail-fast, signal status and COLLECT archive lifecycle')
