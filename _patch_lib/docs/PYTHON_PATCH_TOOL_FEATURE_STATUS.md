# Python Patch Tool v6.17.3 feature status
...
| Windows internal PATCH/COLLECT routing without Bash | **COMPLETE v6.17.3** |
| POSIX fullscreen selector | COMPLETE |
| Windows fullscreen selector (`msvcrt` + VT) + line fallback | **COMPLETE v6.17.3** |
...
| Per-project PATCH mutation serialization (COLLECT/selector remain independent) | **COMPLETE v6.17.3** |
| Exact PATCH package schema + compatibility | COMPLETE |
| Multi-error manifest lint + migration hints | **COMPLETE v6.17.3** |
| Read-only `validate --patch` result classification | **COMPLETE v6.17.3** |
| Aggregate source SHA/existence/anchor diagnostics | **COMPLETE v6.17.3** |
| Sequential data-only OPS dry-run + managed execution timeout | **COMPLETE v6.17.3** |
| Partial-modification detection | COMPLETE |
| Metadata-driven exact-target rollback | COMPLETE |
| Batch effective-target snapshot + rollback verification | **COMPLETE v6.17.3** |
| Whole-batch mutation lock + generation-checked snapshot / POSIX dir-fd restore | **COMPLETE v6.17.3** |
| COLLECT regex isolated worker hard timeout | **COMPLETE v6.17.3** |
| Atomic OPS source write + explicit `already` semantics | **COMPLETE v6.17.3** |
| Windows process-tree timeout/Ctrl+C containment | **COMPLETE v6.17.3** |
| Windows reparse/junction-aware project safety | **COMPLETE v6.17.3** |
...
| Per-run batch summary + aggregate/detail logs | **COMPLETE v6.17.3** |
| Interactive/reopenable `report` browser + history navigation | **COMPLETE v6.17.3** |
...
| Mandatory every-FAIL FAIL_HANDOFF + bounded related-source discovery | **COMPLETE v6.17.3** |
| Machine-readable `PATCH_PACKAGE_CHECKLIST.json` | **COMPLETE v6.17.3** |
...
Batch execution defaults to fail-fast but v6.17.3 also supports explicit `continue_independent`. Continued execution is allowed only after a failure that leaves project state proven clean or successfully rolled back; unsafe partial/tool/rollback failures safety-stop. Dependency failures render successors as `BLOCKED` unless `run_anyway` is explicitly declared.

## Stop condition

v6.17.3 completes persistent batch reporting, aggregate/detail log browsing, diagnostics, Windows robustness/fullscreen parity and final audit scope. No new capability is started automatically after this release.
