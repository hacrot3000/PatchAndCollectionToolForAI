# CODE COLLECTION GUIDE — v6.17.6 AUTHORITATIVE CONTRACT

Python Patch Tool v6.17.7 is self-contained for its documented COLLECT schema. The authoritative action list is not inferred from old guides; it is defined by `COLLECT_ACTION_SCHEMA.json` and enforced before execution.
