...
master=(HERE/'self_test_python_patch_tool_v6_19_5.py').read_text(); assert 'self_test_historical_compatibility_v6_19_5.py' in master and 'self_test_capability_ledger_v6_19_5.py' in master
...
for token in ['COLLECT capability continuity','search_files','symbol_graph']:
    assert token.lower() in policy.lower(),token
assert 'self_test_collect_historical_actions_v6_19_5.py' in master
...
for token in ['Optional controlled installer','user_not_selected','Selection-aware automatic identity adoption']:
    assert token in ledger,token
assert 'self_test_portable_installer_v6_19_5.py' in master

# v6.19.5 additive database SELECT capability is release-gated too.
...
assert (HERE/'self_test_database_select_v6_19_5.py').is_file()
assert (DOC/'DATABASE_SELECT_ACTIVE_BUILDER.md').is_file()
assert (ROOT/'tools'/'db_profiles.example.json').is_file()
for token in ['database_select','active builder','raw SQL','self_test_database_select_v6_19_5.py']:
...
assert (HERE/'self_test_cleartext_companion_v6_19_5.py').is_file()
for token in ['clear-text companion','COLLECT result','FAIL_HANDOFF']:
    assert token.lower() in ledger.lower(), token


# v6.19.5 additive AI tool-context synchronization capability is release-gated.
assert (HERE/'python_patch_ai_sync.py').is_file()
assert (DOC/'AI_TOOL_SYNC_CONTRACT.md').is_file()
assert (HERE/'self_test_ai_sync_v6_19_5.py').is_file()
assert 'ai_context' in collect['request']['allowed_fields']
assert 'ai_context' in schema['manifest']['allowed_fields']
for token in ['AI tool-context synchronization','AI_TOOL_SYNC','sync_token','self_test_ai_sync_v6_19_5.py']:
    assert token.lower() in ledger.lower(), token
assert 'self_test_ai_sync_v6_19_5.py' in master


# v6.19.5 copy-friendly upload-path continuity is release-gated.
assert (HERE/'python_patch_upload_alias.py').is_file()
assert (HERE/'self_test_copyable_upload_path_v6_19_5.py').is_file()
for token in ['copy-friendly','ptv_to_ai','hard-link','self_test_copyable_upload_path_v6_19_5.py']:
    assert token.lower() in ledger.lower(), token
assert 'self_test_copyable_upload_path_v6_19_5.py' in master

print('PASS: v6.19.5 cumulative capability ledger and no-silent-removal governance are packaged and release-gated')
