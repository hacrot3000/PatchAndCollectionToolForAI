# Python Patch Tool v6.19.1 native Windows runtime lane.
...
'@.Replace('__ROOT__', $TestRoot.Replace('\','\\'))
    ...
    & $PythonExe @Prefix (Join-Path $TestRoot 'tools\_patch_lib\self_test_windows_runtime_v6_19_1.py')
    Assert-True ($LASTEXITCODE -eq 0) 'Windows runtime contract self-test failed'

    Write-Host "PASS: Python Patch Tool v6.19.1 native Windows lane (BAT + PowerShell + Unicode/space path + continue batch + report)"
    exit 0
}
finally {
